 ...
 * NOTE: This class has been modified slightly to allow overrides on some of it's parsing routines.
 */
public class JavaScriptCompressor {
	 ...
	 * This buffer may only be modified by invoking the <code>append()</code> method.
	 */
	protected final char[] outputBuffer;
	 ...
	 * @param script
	 */
	protected JavaScriptCompressor(String script) {
		this.script = script.toCharArray();
		outputBuffer = new char[(int) (script.length() * 1.5)];
		outputPos = 0;
	}
		...
		while (!endReached) {
			if (Character.isJavaIdentifierStart((int) ch)) {
				...
				if (ch == '/') {
					...
					append('/');
					// Handle possible regular expression *with* a nasty escape of '\/'
	...
	protected final void append(char ch) {
		outputBuffer[outputPos++] = lastAppend = ch;
	...
	private boolean isWhitespace() {
		return ch == SPACE || ch == TAB || ch == LINE_FEED || ch == CARRIAGE_RETURN;
	}
	 ...
	 */
	protected void renderIdentifier() {
