...
public final class CookieUtils {
	 ...
	 *
	 * Tomcat uses STRICT_SERVLET_COMPLIANCE, whereas this code uses COOKIE_VERSION_ONE_STRICT_COMPLIANCE, which unlike the Tomcat variant not only affects cookie generation, but also cookie parsing. By default, cookies are parsed as v0 cookies, in order
	 * to maintain backward compatibility with GlassFish v2.x
	 */
	...
	/*
	 * List of Separator Characters (see isSeparator()) Excluding the '/' char violates the RFC, but it looks like a lot of people put '/' in unquoted values: '/': ; //47 '\t':9 ' ':32 '\"':34 '\'':39 '(':40 ')':41 ',':44 ':':58 ';':59 '<':60 '=':61
	 * '>':62 '?':63 '@':64 '[':91 '\\':92 ']':93 '{':123 '}':125
	 */
	...
	public static boolean equalsIgnoreCase(String s, byte[] b, int start, int end) {
		...
		int boff = start;
		for (int i = 0; i < blen; i++) {
			final int b1 = AsciiToLower(b[boff++]);
			final int b2 = AsciiToLower(s.charAt(i));
			if (b1 != b2) {
				return false;
			}
		}
	...
	public static boolean equalsIgnoreCase(String s1, String s2, int start, int end) {
		...
		int boff = start;
		for (int i = 0; i < blen; i++) {
			final int b1 = AsciiToLower(s1.charAt(i));
			final int b2 = AsciiToLower(s2.charAt(boff++));
			if (b1 != b2) {
				return false;
			}
		}
	...
	public static BitSet GetInitialSafeChars() {
		return (BitSet) InitialSafeChars.clone();
	}
	...
	static {
		...
		// Dangerous: someone may treat this as " " RFC1738 does allow it, it's not reserved
		// initialSafeChars.set('+'); extra
		InitialSafeChars.set('!');
		InitialSafeChars.set('*');
		InitialSafeChars.set('\'');
		InitialSafeChars.set('(');
