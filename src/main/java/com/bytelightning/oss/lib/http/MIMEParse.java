...
public final class MIMEParse {
	...
	/**
	 * Find the best match for a given mimeType against a list of media_ranges that have already been parsed by MimeParse.parseMediaRange(). Returns a tuple of the fitness value and the value of the 'q' quality parameter of the best match, or (-1, 0) if
	 * no match was found. Just as for quality_parsed(), 'parsed_ranges' must be a list of parsed media ranges.
	 * 
	 * @param mimeType
	 * @param parsedRanges
	 */
	...
	/**
	 * Find the best match for a given mime-type against a list of ranges that have already been parsed by parseMediaRange(). Returns the 'q' quality parameter of the best match, 0 if no match was found. This function bahaves the same as quality() except
	 * that 'parsed_ranges' must be a list of parsed media ranges.
	 * 
	 * @param mimeType
	 * @param parsedRanges
	 * @return
	...
	public static String bestMatch(Collection<String> supported, String header) {
		...
		String retVal = NumberUtils.compare(lastOne.quality, 0) != 0 ? lastOne.mimeType : "";
		return retVal;
	}
